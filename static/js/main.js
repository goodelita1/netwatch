...
function mtPopulateDevices() {
  ...
  sel.innerHTML = '<option value="">— выберите роутер —</option>' +
    allDevices.map(d =>
      `<option value="${d.ip}">${d.name} (${d.ip})${d.cred_login ? '' : ' ⚠️ нет пароля'}</option>`
    ).join('');
  if (cur) sel.value = cur;
...
function mtOpenDeviceCard() {
  ...
  if (!ip) { _setText('mtConnStatus', '⚠️ Сначала выберите устройство'); return; }
  const dev = allDevices.find(d => d.ip === ip);
  if (!dev) { _setText('mtConnStatus', '⚠️ Устройство не найдено в базе'); return; }
...
async function mtConnect() {
  _mtIp = document.getElementById('mtDevSel')?.value || '';
  if (!_mtIp) { _setText('mtConnStatus', '⚠️ Выберите устройство'); return; }
  const st = document.getElementById('mtConnStatus');
  st.textContent = '⟳ Подключение...'; st.style.color = 'var(--muted)';
  ...
  try {
    ...
    if (!crd.has_login || !crd.has_password) {
      st.style.color = 'var(--red)';
      st.innerHTML = `❌ Нет учётных данных. 
        <span style="color:var(--yel)">Откройте карточку устройства → заполните Логин и Пароль</span>`;
      return;
    }
  ...
  try {
    ...
    if (d.ok) {
      ...
      if (err.includes('invalid user') || err.includes('password')) {
        hint = ' — проверьте логин/пароль в карточке устройства';
      } else if (err.includes('закрыт') || err.includes('8728')) {
        hint = ' — выполните на MikroTik: /ip service enable api';
      } else if (err.includes('Таймаут')) {
        hint = ' — роутер недоступен или API выключен';
      }
...
function _mtRenderSessions(sessions) {
  ...
  if (!sessions.length) {
    el.innerHTML = '<div style="color:var(--muted);font-size:11px;padding:8px">Нет активных сессий</div>';
    return;
  }
...
function _mtRenderUsers(users, profiles) {
  ...
  if (!users.length) {
    el.innerHTML = '<div style="color:var(--muted);font-size:11px;padding:8px">Нет пользователей</div>';
    return;
  }
...
function _mtRenderDhcp(leases) {
  ...
  if (!leases.length) {
    tbody.innerHTML = '<tr><td colspan="7" style="text-align:center;padding:20px;color:var(--muted)">Нет лизов</td></tr>';
    return;
  }
...
async function mtLoadSyslog() {
  ...
  if (st) {
    st.textContent = r.running ? '🟢 Работает' : '🔴 Остановлен';
...
function _mtRenderSyslog(entries) {
  ...
  if (!entries.length) {
    el.innerHTML = '<div style="color:var(--muted);padding:10px">Нет записей</div>';
    return;
  }
...
async function mtSyslogStart() {
  ...
  const st = document.getElementById('mtSyslogStatus');
  if (d.ok) {
    st.textContent = '🟢 Работает на порту ' + port;
    st.style.color = 'var(--green)';
